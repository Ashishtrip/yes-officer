...
const biddersData = [
  {
    ...
    risk: "Low Risk",
    status: "Verified",
    summary: "All 5 checks passed (Udyam, GSTN active, PAN matched, No Blacklist, ITR valid)",
    filterType: "verified",
  },
  {
    id: 2,
    ...
    risk: "Medium Risk",
    status: "Issues Found",
    summary: "Udyam enterprise category mismatch (Declared: Micro, Portal: Medium)",
    filterType: "flagged",
  },
  {
    id: 3,
    name: "PQR Traders",
    rank: "Flagged",
    rankClass: "bg-error-container/40 text-error",
    ...
    summary: "GSTIN cancelled / inactive on portal; PAN mismatch detected",
    filterType: "flagged",
  },
  {
    id: 4,
    ...
    rank: "Queued",
    rankClass: "bg-surface-container text-on-surface-variant",
    ...
    risk: "Pending",
    status: "In Progress",
    summary: "EPFO API verification pending queue response (Est. 2 mins)",
    filterType: "pending",
  },
];
...
export default function Dashboard() {
  ...
  const filteredBidders = biddersData.filter((bidder) => {
    const matchesFilter = filter === "all" || bidder.filterType === filter;
    const matchesSearch =
      bidder.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
      bidder.gstin.toLowerCase().includes(searchQuery.toLowerCase()) ||
      bidder.gemId.toLowerCase().includes(searchQuery.toLowerCase());
    ...
    <>
      <header className="fixed top-0 left-0 w-full z-50 bg-surface-container-lowest border-b border-surface-container-high">
        <div className="h-14 w-full px-layout-gutter flex items-center justify-between gap-space-lg">
          ...
          >
            <a
              className="h-full flex items-center px-space-xs font-title-sm border-b-2 border-primary text-primary transition-colors"
              href="#"
            >
              Tenders & Bids
            </a>
            ...
            <div className="bg-surface-container-lowest rounded-xl p-space-md shadow-sm space-y-space-md">
                      ...
                      <tr key={bidder.id} className="hover:bg-surface-container-low/60 transition-colors group">
                        <td className="py-3.5 px-space-md">
                          <div className="flex flex-col">
                            <div className="flex items-center gap-2">
                              ...
                              <span className={`font-label-sm text-label-sm px-1.5 py-0.5 rounded font-medium ${bidder.rankClass || 'bg-surface-container text-on-surface-variant'}`}>{bidder.rank}</span>
                            </div>
                        ...
                        <td className="py-3.5 px-space-md">
                          <div className="flex flex-col gap-1">
                            <div className="flex items-center justify-between font-tabular-num">
                              <span className={`font-semibold ${bidder.score === null ? 'text-outline' : (bidder.score >= 90 ? 'text-emerald-600' : (bidder.score >= 70 ? 'text-amber-600' : 'text-error'))}`}>
                                {bidder.score === null ? '--' : bidder.score} / 100
                              </span>
                              <span className={`font-label-sm text-label-sm ${bidder.score === null ? 'text-outline' : (bidder.score >= 90 ? 'text-emerald-700' : (bidder.score >= 70 ? 'text-amber-700' : 'text-error'))}`}>
                                {bidder.scoreLabel}
                              </span>
                            </div>
                            <div className="w-full bg-surface-container h-2 rounded-full overflow-hidden">
                              <div 
                                className={`h-full rounded-full ${bidder.score === null ? 'bg-outline-variant animate-pulse' : (bidder.score >= 90 ? 'bg-emerald-500' : (bidder.score >= 70 ? 'bg-amber-500' : 'bg-error'))} transition-all duration-500`} 
                                style={{ width: bidder.score === null ? '15%' : `${bidder.score}%` }}
                              ></div>
                            </div>
                          </div>
                        </td>
                        <td className="py-3.5 px-space-md">
                          <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full font-label-sm text-label-sm font-semibold ${bidder.score === null ? 'bg-surface-container text-on-surface-variant' : (bidder.score >= 90 ? 'bg-emerald-50 text-emerald-700' : (bidder.score >= 70 ? 'bg-amber-50 text-amber-700' : 'bg-rose-50 text-rose-700'))}`}>
                            <span className={`w-1.5 h-1.5 rounded-full ${bidder.score === null ? 'bg-outline' : (bidder.score >= 90 ? 'bg-emerald-500' : (bidder.score >= 70 ? 'bg-amber-500' : 'bg-rose-500'))}`}></span>
                            {bidder.risk}
                          </span>
                        </td>
                        <td className="py-3.5 px-space-md">
                          <span className={`inline-flex items-center gap-1 font-medium ${bidder.score === null ? 'text-on-surface-variant' : (bidder.score >= 90 ? 'text-emerald-700' : (bidder.score >= 70 ? 'text-amber-700' : 'text-error'))}`}>
                            <span className={`material-symbols-outlined text-[18px] ${bidder.score === null ? 'animate-spin' : ''}`}>
                              {bidder.score === null ? 'progress_activity' : (bidder.score >= 90 ? 'check_circle' : (bidder.score >= 70 ? 'warning' : 'cancel'))}
                            </span>
                            {bidder.status}
                          </span>
                        </td>
                        <td className="py-3.5 px-space-md">
                          <div className="flex items-start gap-1.5">
                            <span className={`material-symbols-outlined text-[18px] shrink-0 mt-0.5 ${bidder.score === null ? 'text-outline' : (bidder.score >= 90 ? 'text-emerald-600' : (bidder.score >= 70 ? 'text-amber-600' : 'text-error'))}`}>
                              {bidder.score === null ? 'hourglass_empty' : (bidder.score >= 90 ? 'verified_user' : (bidder.score >= 70 ? 'report_problem' : 'error'))}
                            </span>
                            <span className="text-on-surface-variant">{bidder.summary}</span>
                          </div>
                        </td>
                        <td className="py-3.5 px-space-md text-right">
                          <button 
                            className={`inline-flex items-center justify-center px-3 py-1.5 rounded-xl font-label-md text-label-md transition-colors ${bidder.score === null ? 'bg-surface-container-low text-outline opacity-60 cursor-not-allowed' : 'bg-surface-container-low text-on-surface hover:bg-surface-container-high'}`} 
                            type="button"
                            disabled={bidder.score === null}
                          >
                            View Details
                          </button>
                        </td>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-space-sm pt-space-xs font-body-sm text-body-sm text-on-surface-variant">
                <div className="font-tabular-num">
                  Showing <span className="font-semibold text-on-surface">1 to {filteredBidders.length}</span> of <span className="font-semibold text-on-surface">{biddersData.length}</span> bidders
                </div>
